...
class RejoinCandidateGenerator(val route: ScheduledRoute, val anchor: Double,
    ...
    val minimumSafeRejoinProgress: Double? = null) {
    ...
    fun problem(progress: Double): String? = when {
        ...
        minimumSafeRejoinProgress != null && progress < minimumSafeRejoinProgress -> "規制区間の終端より先の復帰地点を選択してください"
    ...
    fun generate(): List<RejoinTarget> {
        ...
        for (offset in config.preferredOffsetsMeters.sorted()) {
            val progress = maxOf(anchor + offset, minimumSafeRejoinProgress ?: 0.0)
