...
fun deviationUiState(snapshot: RouteDeviationSnapshot): DeviationUiState = when {
    snapshot.matchQuality == RouteMatchQuality.UNRELIABLE && snapshot.state == RouteDeviationState.OFF_ROUTE ->
        DeviationUiState("所定経路から外れている可能性があります・位置情報を確認中", true)
    snapshot.state == RouteDeviationState.ON_ROUTE -> DeviationUiState()
    snapshot.state == RouteDeviationState.SUSPECTED_OFF_ROUTE -> DeviationUiState("所定経路との位置関係を確認中")
    snapshot.state == RouteDeviationState.OFF_ROUTE -> DeviationUiState("所定経路から外れている可能性があります", true)
    snapshot.state == RouteDeviationState.RECOVERING -> DeviationUiState("所定経路への復帰を確認中")
