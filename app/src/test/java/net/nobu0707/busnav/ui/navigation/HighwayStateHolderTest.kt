...
class HighwayStateHolderTest {
    @Test fun accuracyDisabledLayoutAndRouteReplacementKeepSafeState() {
        ...
        try {
            val holder = NavigationStateHolder(provider, object : ScheduledRouteRepository { override suspend fun getActiveRoute() = route }, scope, Dispatchers.Unconfined)
            holder.setPermission(LocationPermissionState.Granted)
            fun position(accuracy: Float, lon: Double = .009) { flow.value = LocationUpdate.Position(LocationState(GeoPoint(0.0, lon), accuracy, null, null, 1)) }
            position(5f)
