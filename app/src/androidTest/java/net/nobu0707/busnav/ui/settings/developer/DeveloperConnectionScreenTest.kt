...
class DeveloperConnectionScreenTest {
    ...
    private fun exercise(orientation: Int) {
        ...
        rule.onNodeWithTag("basemap_region_kanto").assertDoesNotExist()
        rule.onNodeWithTag("basemap_region_chubu").assertDoesNotExist()
        ...
        rule.runOnIdle { assertEquals(net.nobu0707.busnav.map.basemap.BasemapRegion.JAPAN, repository.settings.value.basemapRegion) }
        ...
        rule.onNodeWithTag("basemap_status").performScrollTo().assertTextEquals("HTTPエラー (404)：全国地図データがありません")
