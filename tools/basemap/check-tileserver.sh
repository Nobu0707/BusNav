...
for region in kanto chubu; do
  curl --fail --silent --show-error "$base_url/data/$region.json" >/dev/null
  curl --fail --silent --show-error "$base_url/styles/busnav-$region/style.json" >/dev/null
  curl --fail --silent --show-error "$base_url/styles/busnav-$region-light/style.json" >/dev/null
done
curl --fail --silent --show-error "$base_url/data/kanto/10/909/403.pbf" >/dev/null
...
echo "PASS Kanto + Chubu Light/Dark styles, TileJSON and representative PBF tiles"
