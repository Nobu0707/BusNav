...
HEADING_UP follows resolved course, NORTH_UP follows position with bearing 0. First centering uses the existing zoom only without a restored camera; toggles and recenter preserve zoom. All follow updates remain top-down. Manual drag/rotate/zoom suspends follow without changing preference. Current location restores selected orientation. Preview overview requests cannot override an active following camera.
