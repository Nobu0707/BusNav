...
- Emulator shard validation exposed an editor drag regression: null-location updates canceled native transitions even outside navigation. Cancellation is now restricted to active navigation follow, and native gesture callbacks simply suspend follow. Both editor runtime tests pass after the fix; final suites recheck navigation drag/recenter and all editor flows.
