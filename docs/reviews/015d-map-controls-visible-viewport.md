...
| SOG06 Physical Android / Android 14 | 1 | Gradle connected suite PASS | 74 cases: 58 executed, 16 remote-map reachability assumptions on Wi-Fi. Remote routing cases passed. No retry. A separate one-time cellular manual check rendered the nationwide remote map. |
...
Physical one-round manual smoke: the scale button changed to NORMAL and the ruler to 200 m. The Route Editor cursor appeared at the visible map center; tapping register increased the point count from zero to one. On Wi-Fi, the remote Japan map did not render and showed the disconnected banner, matching the 16 connected-test reachability assumptions. One subsequent mobile-network check rendered the nationwide map with Kansai, Chubu and Kanto in view; the original Wi-Fi state was restored immediately afterward. Remote routing tests passed. No local server was used.
