...
Phase 002 の画面は完成版ナビゲーションではなく、地図、所定経路、案内、運行情報、補助操作を段階的に実装するための土台です。
...
運行情報には `所定経路：<route name>`、未選択時は `所定経路：未選択` を表示します。補助操作は誤操作を避けるため無効なプレースホルダのままです。実装時は走行中操作制限、押下領域、警告色を合わせて設計します。
