...
Phase 010.6D では地図右側に方位、縮尺プリセット、投影ベースの距離目盛をまとめます。ナビ中は既存の方位切替を表示し、その他の地図で北から4°以上回転した場合だけ「北を上に戻す」を表示します。HEADING_UP の自車は実可視領域の上から85%、下から15%です。Route Editor のカーソルと登録地点はシートを除いた可視中心を使用します。[地図操作](../map-controls.md)、[可視領域](../map-visible-viewport.md)。
